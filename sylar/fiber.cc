...
namespace sylar
{
    ...
    static std::atomic<uint64_t> s_fiber_count{0}; // 全局协程数量

    static thread_local Fiber *t_fiber = nullptr;           // 当前正在执行的
    ...
    void Fiber::swapOut()
    {
        ...
        if (swapcontext(&m_ctx, &Scheduler::GetMainFiber()->m_ctx))// 获取调度协程的上下文，切换到调度协程
        {
            SYLAR_ASSERT2(false, "swapcontext");
        }
    }
