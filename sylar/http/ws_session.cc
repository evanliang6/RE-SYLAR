...
HttpRequest::ptr WSSession::handleShake() {// 握手
    HttpRequest::ptr req;
    do {
        req = recvRequest();// 先等待客户端发送握手请求
        if(!req) {
            SYLAR_LOG_INFO(g_logger) << "invalid http request";
            break;
        }
        if(req->getHeaderAs<int>("Sec-webSocket-Version") != 13) {// 版本必须是13
        ...
        std::string v = key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11";// 拼接魔术字符串
...
int32_t WSSendMessage(Stream* stream, WSFrameMessage::ptr msg, bool client, bool fin) {
    do {
        ...
        ws_head.mask = client;// 客户端发送的数据必须mask
        ...
        if(size < 126) {
            ...
            ws_head.payload = 127;// 0-125是直接表示长度，126表示后面2字节表示长度，127表示后面8字节表示长度
        }
