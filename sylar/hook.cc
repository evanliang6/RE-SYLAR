...
#define XX(name) name ## _f = (name ## _fun)dlsym(RTLD_NEXT, #name);// 从动态链接库中获取原始函数的地址。例如sleep_f=(sleep_fun)dlsym(RTLD_NEXT, "sleep");
    HOOK_FUN(XX);
#undef XX
}
...
static ssize_t do_io(int fd, OriginFun fun, const char* hook_fun_name,
        uint32_t event, int timeout_so, Args&&... args) {// 统一处理需要hook的IO函数。
...
retry:
    ssize_t n = fun(fd, std::forward<Args>(args)...);// 调用原始IO函数.返回值有效直接结束
    while(n == -1 && errno == EINTR) {// 被信号中断，重新调用
        n = fun(fd, std::forward<Args>(args)...);
    }
    if(n == -1 && errno == EAGAIN) {// 需要等待IO事件就绪
        ...
        if(to != (uint64_t)-1) {// 设置了超时
            timer = iom->addConditionTimer(to, [winfo, fd, iom, event]() {
                ...
                iom->cancelEvent(fd, (sylar::IOManager::Event)(event));// 取消事件
            }, winfo);
        }
        ...
        if(SYLAR_UNLIKELY(rt)) {
            ...
            sylar::Fiber::YieldToHold();// 如果因为事件就绪被唤醒，回到retry继续执行fun()
            if(timer) {
                timer->cancel();
            }
            if(tinfo->cancelled) {
...
int socket(int domain, int type, int protocol) {
    ...
    sylar::FdMgr::GetInstance()->get(fd, true);// 创建FdCtx
    return fd;
}
